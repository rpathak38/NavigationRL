...
class ConfigurationSaver:
    def __init__(self, log_dir, save_items, pretrained_items=None):
        ...
        if pretrained_items is not None:
            ...
            pretrained_data_dir = self._data_dir + '/pretrained_data'
            os.makedirs(pretrained_data_dir)
            for pretrained_item in pretrained_items:
                base_file_name = ntpath.basename(pretrained_item)
                copyfile(pretrained_item, pretrained_data_dir + '/' + base_file_name)
...
def TensorboardLauncher(directory_path):
    ...
    tb.configure(argv=[None, '--logdir', directory_path, '--port', '24681', '--host', '0.0.0.0'])
    url = tb.launch()
